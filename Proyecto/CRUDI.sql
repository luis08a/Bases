...
CREATE OR REPLACE PACKAGE BODY PC_PATROCINADORES IS
    ...
    BEGIN
        INSERT INTO PATROCINADORES (tipoDeDocumento, numeroDeDocumento, nombre) VALUES (P_tipoDoc, P_nDoc, P_nombre);
        COMMIT;
        EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK;
    ...
    BEGIN
        DELETE FROM PATROCINADORES WHERE tipoDeDocumento = P_tipoDoc and numeroDeDocumento = P_nDoc;
        COMMIT;
        EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK;
    ...
    BEGIN
    OPEN CON_EVENTOS_PATROCINADORES FOR
        SELECT nombre, fechaInicio, fechaFin, detalles FROM EVENTOS;
    RETURN CON_EVENTOS_PATROCINADORES;
    END;
    FUNCTION CO_MAYORPATROCINIO RETURN SYS_REFCURSOR IS MAYORPATROCINIO SYS_REFCURSOR;
    BEGIN
    OPEN MAYORPATROCINIO FOR
        SELECT Patrocinadores.nombre from Patrocinadores join Patrocina ON (Patrocinadores.tipoDeDocumento=tipoDeDocumentoPatrocinador and numeroDeDocumentoPatrocinador=numeroDeDocumento) group by Patrocinadores.nombre having max(monto)=(select max(monto) from patrocina);
    RETURN MAYORPATROCINIO;
    END;
...
CREATE OR REPLACE PACKAGE BODY PC_Empleados IS
    ...
    BEGIN
        INSERT INTO DEPARTAMENTOS (nombre, descripcion) VALUES (DEP_nombre, DEP_Desc);
        COMMIT;
        EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK;
    ...
    BEGIN
        INSERT INTO EMPLEADOS (nombres, apellidos, telefono, cargo, departamento) VALUES (EMP_nombres, EMP_apellidos, EMP_tel, emp_cargo, EMP_DEP);
        COMMIT;
        EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK;
    ...
    BEGIN
        UPDATE empleados set CARGO=emp_CARGO where emp_id=id;
        COMMIT;
        EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK;
            RAISE_APPLICATION_ERROR(-20010, 'Error al actualizar el cargo del empleado.');
    END;
    
    FUNCTION CO_EMPXDEP RETURN SYS_REFCURSOR IS EMPXDEP SYS_REFCURSOR;
    BEGIN
    OPEN EMPXDEP FOR
        SELECT departamento,count(ID) FROM EMPLEADOS GROUP BY DEPARTAMENTO;
    RETURN EMPXDEP;
    END;
        
    FUNCTION CO_EMPXSALA RETURN SYS_REFCURSOR IS EMPXSALA SYS_REFCURSOR;
    BEGIN
    OPEN EMPXSALA FOR
        SELECT sala,count(EMPLEADO) AS CANTIDAD_DE_EMPLEADOS FROM empleadostrabajanensalas GROUP BY SALA;
    RETURN EMPXSALA;
    END;
